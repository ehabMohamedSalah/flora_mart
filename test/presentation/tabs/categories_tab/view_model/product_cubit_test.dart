...
@GenerateMocks([GetAllProductsUsecase,FilterUsecase,SearchUsecase])
void main() {
  group('ProductCubit', () {
    ...
    const String testFilter = 'Old';
    const String testSearchQuery = 'Red';

    setUp(() {
      getAllProductsUsecase = MockGetAllProductsUsecase();
      filterUsecase = MockFilterUsecase();
      searchUsecase = MockSearchUsecase();
      productCubit = ProductCubit(getAllProductsUsecase,filterUsecase,searchUsecase);
    ...
    blocTest<ProductCubit, ProductState>(
      ...
      build: () {
        ...
        when(getAllProductsUsecase.execute(testTypeId, testType)).thenAnswer(
            (_) async => ErrorApiResult(Exception('Failed to load products')));
        return productCubit;
      },
      act: (cubit) => cubit.doIntent(GetProductsIntent(testTypeId, testType)),
      expect: () => [
    ...
    blocTest<ProductCubit, ProductState>(
      'emits [ProductLoadingState, ProductLoadedState] when getProductsBasedOnFilter is successful',
      build: () {
        // Mock the result of the use case
        when(filterUsecase.execute(testFilter))
            .thenAnswer((_) async => SuccessApiResult([
          Products(
              id: "673e1cd711599201718280fb",
              title: "Wdding Flower",
      ...
      act: (cubit) => cubit.doIntent(GetProductsBasedOnFilterIntent(testFilter)),
      expect: () => [
        isA<ProductLoadingState>(),
        isA<ProductLoadedState>(),
      ],
    ...
    blocTest<ProductCubit, ProductState>(
      'emits [ProductLoadingState, ProductErrorState] when getProductsBasedOnFilter fails',
      build: () {
        // Mock the error result of the use case
        when(filterUsecase.execute(testFilter)).thenAnswer(
                (_) async => ErrorApiResult(Exception('Failed to load products')));
        return productCubit;
      },
      act: (cubit) => cubit.doIntent(GetProductsBasedOnFilterIntent(testFilter)),
    ...
    blocTest<ProductCubit, ProductState>(
      ...
      build: () {
        // Mock the result of the use case
        when(searchUsecase.execute(testSearchQuery))
            .thenAnswer((_) async => SuccessApiResult([
          Products(
              id: "673e1cd711599201718280fb",
              title: "Wdding Flower",
      ...
      act: (cubit) => cubit.doIntent(GetProductsBasedOnSearchQueryIntent(testSearchQuery)),
      expect: () => [
        isA<ProductLoadingState>(),
        isA<ProductLoadedState>(),
      ],
    ...
    blocTest<ProductCubit, ProductState>(
      ...
      build: () {
        // Mock the error result of the use case
        when(searchUsecase.execute(testSearchQuery)).thenAnswer(
                (_) async => ErrorApiResult(Exception('Failed to load products')));
        return productCubit;
      },
      act: (cubit) => cubit.doIntent(GetProductsBasedOnSearchQueryIntent(testSearchQuery)),
