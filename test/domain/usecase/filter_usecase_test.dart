...
void main() {
  ...
  setUp(() {
    ...
    filterUsecase = FilterUsecase(mockFilterRepo);
  });
  ...
  group('FilterUsecase Tests', () {
    ...
    test(
        'should return SuccessApiResult when products are retrieved successfully',
          ...
          when(mockFilterRepo.getAllProductsBasedOnFilter(testFilter))
              .thenAnswer((_) async => SuccessApiResult(productList));

          // Act
          final result = await filterUsecase.execute(testFilter);

          // Assert
          expect(result, isA<SuccessApiResult<List<Products>>>());
          expect((result as SuccessApiResult).data, isNotEmpty);
          //expect(result.data!.length, 2);
          verify(mockFilterRepo.getAllProductsBasedOnFilter(testFilter))
              .called(1);
          verifyNoMoreInteractions(mockFilterRepo);
        });
